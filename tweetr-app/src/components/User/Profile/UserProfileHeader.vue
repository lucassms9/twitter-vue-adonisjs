    ...
    <div class="ui grid container">
      ...
      <div class="bottom aligned twelve wide column">
          ...
          <div class="item">
            <div class="content">
              <router-link :to="isOwner ? `/followers` : `/${user.username}/followers`">
                <div class="header">Seguidores</div>
        ...
        <div class="ui right floated horizontal list" v-if="isOwner">
          <router-link to="/settings/profile" class="ui button primary">Editar Perfil</router-link>
        </div>
        <div class="ui right floated horizontal list" v-if="isLoggedIn && !isOwner">
          <button class="ui tiny primary button" v-if="isFollowing" @click="unFollow(user.id)">Deixar de seguir</button>
          <button class="ui tiny primary button" v-else @click="follow(user.id)">Seguir</button>
        </div>
      </div>
    </div>
  </div>
